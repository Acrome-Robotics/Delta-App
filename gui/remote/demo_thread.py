        ...
        try:
            if self.dev is not None:
                if self.mode == self.MODE_CIRCLE:
                    ...
                    ix, iy, iz = self.cx, self.cy, self.circle_z
                else:
                    self.status_msg.emit("Starting Square demo...")
                    ix, iy, iz = self.Rect4[0], self.Rect4[1], self.Rect4[2]
                
                self.dev_mutex.lock()
                self.dev.move_pos(ix, iy, iz)
        ...
        while self._running:
            try:
                ...
                if self.mode == self.MODE_CIRCLE:
                    ...
                    if self.dev is not None:
                        self.dev_mutex.lock()
                        self.dev.move_pos(circlePos[0], circlePos[1], circlePos[2])
                        self.dev_mutex.unlock()
                    ...
                    for i in range(4):
                        ...
                        while True:
                            ...
                            if self.dev is not None:
                                self.dev_mutex.lock()
                                self.dev.move_pos(traj_pos[0], traj_pos[1], traj_pos[2])
                                self.dev_mutex.unlock()
